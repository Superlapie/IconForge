...
func resolve(purpose_id: String, inspection: Dictionary, hints: Dictionary = {}) -> Dictionary:
	...
	var preset_id: String = _resolve_preset_id(purpose_id, purpose_def, hints)
	...
	if not purposes.preset_matches_purpose_contract(purpose_id, preset):
		return _failure("PRESET_PURPOSE_MISMATCH", "Preset '%s' does not satisfy purpose '%s' output contract." % [preset_id, purpose_id], purpose_id)
	...
	var reasons: Array[String] = [
		...
		"preset=%s" % preset_id,
		"asset_class=%s" % str(hints.get("asset_class", "automatic")),
	]
...
func _resolve_preset_id(purpose_id: String, purpose_def: Dictionary, hints: Dictionary) -> String:
	match purpose_id:
		"inventory_icon":
			return _resolve_inventory_preset(hints, purpose_def)
		"equipment_preview", "shop_thumbnail", "neutral_thumbnail":
...
func _resolve_inventory_preset(hints: Dictionary, purpose_def: Dictionary) -> String:
	var asset_class: String = str(hints.get("asset_class", "automatic"))
	match asset_class:
		"weapon":
			return "weapon"
		"armor":
...
func _apply_safe_hints(hints: Dictionary, morphology: String) -> Dictionary:
	var patch: Dictionary = {}
	var orientation_hint: String = str(hints.get("orientation_hint", "automatic"))
	if orientation_hint == "automatic":
		orientation_hint = _default_orientation_for_morphology(morphology)
	if orientation_hint != "automatic":
	...
	if framing_bias != "automatic":
		...
		if not patch.has("camera"):
			patch["camera"] = {}
		patch["camera"]["occupancy"] = clampf(0.82 + occupancy_delta, 0.55, 0.95)
	return patch

func _default_orientation_for_morphology(morphology: String) -> String:
	match morphology:
		"elongated":
			return "diagonal"
		"flat_wide":

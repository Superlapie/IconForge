...
func render_batch(input_path: String, preset: PresetDefinition, output_dir: String, options: Dictionary = {}) -> Dictionary:
	...
	for source_path in sources:
		var filename: String = _expand_pattern(pattern, source_path, preset.get_id())
		...
		var result: Dictionary = await render_service.render(source_path, preset, options.get("override", {}), output_path, force)
	...
	if manifest_enabled:
		var manifest: Dictionary = {
			...
			"summary": {"total": sources.size(), "success": success_count, "failed": failure_count},
			"renders": renders
		}
